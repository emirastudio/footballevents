...
export default async function NewVenuePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <h1 className="font-[family-name:var(--font-manrope)] text-2xl font-bold text-[var(--color-foreground)]">{t("newTitle")}</h1>
      <p className="mt-1 text-sm text-[var(--color-muted-strong)]">{t("newSubtitle")}</p>
      ...
      <div className="mt-6 rounded-[var(--radius-2xl)] border border-[var(--color-border)] bg-[var(--color-surface)] p-6 shadow-[var(--shadow-sm)]">
        <VenueForm
          ...
          defaults={{ countryCode: organizer.countryCode ?? undefined, city: organizer.city ?? undefined }}
